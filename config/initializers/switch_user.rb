SwitchUser.setup do |config|
  ...
  config.controller_guard = lambda { |current_user, request| Rails.env.development? or Rails.env == "production" and current_user.super_admin? }

  # view_guard is a block,
  # if it returns true, the switch user select box will be shown,
  # else the select box will not be shown
  # if you switch from admin to "user", the current_user param is "user"
  config.view_guard = lambda { |current_user, request| Rails.env.development? or Rails.env == "production" and current_user.super_admin? }
  ...
  config.controller_guard = lambda { |current_user, request, original_user|
    current_user && current_user.super_admin? || original_user && original_user.super_admin?
  }
end
