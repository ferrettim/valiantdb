  ...
  xml.channel do
    ...
    for article in @feed_posts
      xml.item do
        ...
        if article.category == "Paperback"
          text = article.title.to_s + " Volume " + article.issue.to_s + " Trade Paperback written by " + article.writer.to_s + ", with art by " + article.artist.to_s
        elsif article.category == "Hardcover"
          text = article.title.to_s + " Deluxe Hardcover Volume " + article.issue.to_s + " written by " + article.writer.to_s + ", with art by " + article.artist.to_s
        elsif article.category == "Default"
          text = article.title.to_s + " #" + article.issue.to_s + " written by " + article.writer.to_s + ", with art by " + article.artist.to_s + ". This book has " + Book.where(:title => article.title.to_s).where(:issue => article.issue.to_s).where(:rdate => article.rdate.to_s).where(:category => "Variant").count.to_s + " variants."
        end
